...
namespace BusinessLogic.Services.Interfaces
{
    public interface IAccountService
    {
        ...
        Task<(bool IsDone, string Message)> SignUpAsync(RegistrationModel model, string clientAppHost);
        Task<bool> SignOutAsync();
        Task<(bool IsDone, string Message)> ConfirmEmailAsync(string email, string token);
        Task<(bool IsDone, string Message)> SendConfirmEmailAsync(string email, string clientAppHost);
